...
export class AuthService {
  ...
  signUp = async (email, password, passwordConfirm, name) => {
    if (!email) {
      throw new ErrorHandler('이메일을 입력해주세요', 400);
    }
    if (!password) {
      throw new ErrorHandler('비밀번호를 입력해주세요', 400);
    }
    if (!passwordConfirm) {
      throw new ErrorHandler('비밀번호 확인을 입력해주세요', 400);
    }
    if (!name) {
      throw new ErrorHandler('이름을 입력해주세요', 400);
    }
    // 1-2. 이메일 형식에 맞지 않는 경우 - “이메일 형식이 올바르지 않습니다.”
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/; // 정규표현식으로 이메일형태(아이디.@주소.com)를 생각하면됨
    if (!emailRegex.test(email)) {
      throw new ErrorHandler('이메일 형식이 올바르지 않습니다.', 400);
    }
    // 1-3. 동일한 이메일을 가진 사용자가 있는지 확인합니다.
    const isExistUser = await this.usersRepository.findByEmail(email);
    if (isExistUser) {
      throw new ErrorHandler('이미 가입 된 사용자입니다.', 400);
    }

    // 1-4 비밀번호가 6자리 미만인 경우 - “비밀번호는 6자리 이상이어야 합니다.”
    if (password.length < 6) {
      throw new ErrorHandler('비밀번호는 6자리 이상이어야 합니다.', 400);
    }

    // 1-5 비밀번호와 비밀번호 확인이 일치하지 않는 경우 - “입력 한 두 비밀번호가 일치하지 않습니다.”
    if (password !== passwordConfirm) {
      throw new ErrorHandler('입력 한 두 비밀번호가 일치하지 않습니다.', 400);
    }
  ...
  Login = async (email, password) => {
    ...
    if (!email) {
      throw new ErrorHandler('이메일을 입력해 주세요.', 400);
    }
    if (!password) {
      throw new ErrorHandler('비밀번호을 입력해 주세요.', 400);
    }
    ...
    if (!user || !(await bcrypt.compare(password, user.password))) {
      throw new ErrorHandler('인증 정보가 유효하지 않습니다.', 401);
    }
  ...
  getMe = async (userId) => {
    ...
    if(!user) {
      throw new ErrorHandler('사용자를 찾지 못했습니다.', 400);
    }
    return {
      id: user.id,
      email: user.email,
