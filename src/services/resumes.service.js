...
export default class ResumesService {
  ...
  createdResume = async (title, about_me, userId) => {
    ...
    if (!title) {
      throw new ErrorHandler('제목을 입력해 주세요.', 400);
    }
    if (!about_me) {
      throw new ErrorHandler('자기소개를 입력해 주세요.', 400);
    }
    // 1-2 자기소개 글자 수가 150자 보다 짧은 경우
    if (about_me.length < 150) {
      throw new ErrorHandler('자기소개는 150자 이상 작성해야 합니다.', 400);
    }
    // 1-3 사용자 id는 미들웨어를 통해 req.user.id에 설정됨
    if (!userId) {
      throw new ErrorHandler('인증된 사용자가 아닙니다.', 401);
    }
    // 1-4 새로운 이력서 생성
